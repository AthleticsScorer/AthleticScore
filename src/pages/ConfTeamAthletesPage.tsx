import { Button, Heading, VStack, HStack, Input, SimpleGrid, Tabs, TabList, TabPanels, Tab, TabPanel, Box } from "@chakra-ui/react";
        ...
        if (competitionId) {
          axios
            ...
            .then((response) => {
              ...
              setAgeGroups(Array.from(new Set(events.map((e) => e.age_group))));
              setEventStrings(Array.from(new Set(events.map((e) => e.event_name))));
            })
            .catch((error) => {
    ...
    useEffect(() => {
      const fetchTeamName = async () => {
        try {
          ...
          setTeamName(response.data.age_group);
        } catch (error) {
                    ...
                    {ageGroups.map((ag) => (
                        <TabPanel>
                          <SimpleGrid columns={eventStrings.length} spacing={4}>
                            ...
                            {events.filter((e) => e.age_group === ag).map((e, index) => (
                              <HStack padding="10px">
                              {(index % eventStrings.length == 0) ? <Box>
                                {e.event_type}
                              </Box> : <></>}
                              <Box>
                                <Input
